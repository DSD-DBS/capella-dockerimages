...
if __name__ == "__main__":
    ...
    download_url = CAPELLA_DOWNLOAD_URL.format(f"{capella_archive_path}{archive_name}")

    response = requests.get(download_url)
    response.raise_for_status()
    download_path = pathlib.Path("/opt/capella.tar.gz").write_bytes(response.content)
